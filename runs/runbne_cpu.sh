...
# Showing an example run for exercising BNE capability on the CPU (or MPS on Macbooks)
...
python -m scripts.tok_train --max-chars=2000000000 --tokenizer-type=BNE --max-ngram-length=8
